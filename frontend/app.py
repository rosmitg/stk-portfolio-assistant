...
def backend_upload(path: str, file_bytes: bytes, filename: str):
    try:
        ...
        return None, "Request timed out — the upload is taking too long."
    except requests.exceptions.HTTPError as e:
        return None, f"Backend returned {e.response.status_code}: {e.response.text}"
    except Exception as e:
        return None, f"Unexpected error: {e}"
...
def _render_dashboard(df: pd.DataFrame) -> None:
    ...
    with col_bar:
        ...
        fig_bar = go.Figure(go.Bar(
            x=df["ticker"], y=df["gain_loss_pct"],
    ...
    display_df = df.rename(columns={
        ...
        "market_value": "Market Value", "gain_loss": "Gain / Loss", "gain_loss_pct": "G/L %",
    }).copy()
...
with tab_portfolio:
    port_data, port_err = backend_get("/api/v1/portfolio")

    if port_err:
        st.markdown(f'<div class="error-box">⚠️ {port_err}</div>', unsafe_allow_html=True)

    elif port_data:
        _render_dashboard(pd.DataFrame(port_data))
        ...
        <div style="text-align:center; padding:2rem 0 1.5rem;">
            <div style="font-size:1.1rem; font-weight:600; color:#E6EDF3;">
                Add your portfolio to get started
            </div>
            <div style="font-size:0.85rem; color:#8B949E; margin-top:0.4rem;">
                Choose how you want to import your holdings
            </div>
        </div>
        """, unsafe_allow_html=True)

        col_alpaca, col_csv, col_manual = st.columns(3, gap="large")

        # ── Connect Alpaca ────────────────────────────────────────────────────
        with col_alpaca:
            st.markdown("""
            <div style="background:#161B22;border:1px solid #30363D;border-radius:10px;
                        padding:1.25rem;min-height:180px;">
                <div style="font-size:1rem;font-weight:600;color:#E6EDF3;margin-bottom:0.4rem;">
                    Connect Alpaca
                </div>
                <div style="font-size:0.82rem;color:#8B949E;margin-bottom:1rem;">
                    Pull live positions from your Alpaca paper trading account automatically.
                </div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
            if st.button("Sync from Alpaca", use_container_width=True):
                with st.spinner("Syncing positions…"):
                    result, sync_err = backend_post("/api/v1/portfolio/sync-alpaca", {})
                if sync_err:
                    st.error(sync_err)
                else:
        ...
        with col_csv:
            ...
            <div style="background:#161B22;border:1px solid #30363D;border-radius:10px;
                ...
                <div style="font-size:0.82rem;color:#8B949E;margin-bottom:0.5rem;">
                    ...
                    <code>ticker</code>, <code>shares</code>, <code>avg_cost</code>
        ...
        with col_manual:
            ...
            <div style="background:#161B22;border:1px solid #30363D;border-radius:10px;
                ...
                <div style="font-size:0.82rem;color:#8B949E;margin-bottom:0.5rem;">
                    Enter a single holding by ticker, shares, and average buy price.
                </div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
            with st.form("add_holding_form", clear_on_submit=True):
                m_ticker = st.text_input("Ticker", placeholder="AAPL")
                m_name = st.text_input("Company name (optional)", placeholder="Apple Inc.")
                m_shares = st.number_input("Shares", min_value=0.001, step=0.001, format="%.3f")
                m_avg_cost = st.number_input("Avg buy price ($)", min_value=0.01, step=0.01, format="%.2f")
                m_submitted = st.form_submit_button("Add Holding", use_container_width=True)
            if m_submitted:
                if not m_ticker.strip():
                    ...
                    payload = {
                        ...
                        "name": m_name.strip() or m_ticker.strip().upper(),
                        "shares": m_shares,
                        "avg_cost": m_avg_cost,
                    }
                    result, add_err = backend_post("/api/v1/portfolio", payload)
                    if add_err:
                        st.error(add_err)
                    else:
